    ...
    <form class="form-horizontal" role="form" id="form_add" method="post" action="{{url('/unit_tenant/'. $id)}}">
        ...
        <div class="tab-content">
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <select class="chosen-select form-control" name="id_tenant_category" id="form-field-select-3" value='{{$data->nama_tenant_category}}' disabled>
                        <option value=""> </option>
                        @foreach ($data_master_category as $item2)
                        @php
                        $selected = ($item2->id_tenant_category==$data->id_tenant_category)?'selected':'';
                        @endphp
                        <option value="{{$item2->id_tenant_category}}" {{ $selected }}>{{$item2->nama_tenant_category}}
                        </option>
                        @endforeach
                    </select>
                </div>
