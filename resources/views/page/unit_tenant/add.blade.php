    ...
    <form class="form-horizontal" role="form" id="form_add" method="post" action="{{url('/unit_tenant')}}">
        ...
        <div class="tabbable">
            ...
            <div class="form-group">
                <label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Tenant Category </label>

                <div class="col-sm-9">
                    <select class="chosen-select form-control" name="id_tenant_category" id="form-field-select-3"
                        data-placeholder="Choose Tenant Category...">
                        <option value=""> </option>
                        @foreach ($tenantcategorys as $item)
                        <option value="{{$item->id_tenant_category}}">{{$item->nama_tenant_category}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
