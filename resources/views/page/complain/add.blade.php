...
<div class="col-xs-12">
    <!-- PAGE CONTENT BEGINS -->
    <form class="form-horizontal" role="form" id="form_add" method="post" action="{{url('/complain')}}">
        @csrf
        ...
        @endforeach
        @endif
        <input type="hidden" name="id_complain_status" id="id_complain_status" value="1" />

        <div class="form-group">
            <label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Concierge Category </label>
            <div class="col-sm-7">
                @foreach ($namaCategoryComplain as $id=>$item)
                <div class="checkbox" id='chx{{ $item->kode_complain_category }}'>
                    <label>
                        <input type="checkbox" value="{{ $item->id_complain_category }}" id="{{ $item->kode_complain_category }}"
                            name="complainCategory[{{$id}}][id]" onclick="change(this)">
                        {{ $item->nama_complain_category }}
                    </label>
                    <textarea id="description{{ $item->kode_complain_category }}" name="complainCategory[{{$id}}][description]"
